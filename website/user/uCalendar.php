...
?>
    ...
    <div class="container">
        <div id="scheduler">
            <!--main container for the scheduler, it is initialize in here after function is called on line-->
            ...
            <script>
                ...
                let datasourcedata = ds.data();             //data from the datasource is stored in a temporary array,
                let startingDates = [];                 //in this for loop i pull the starting dates from the datasource and store them in this array
                for (let i = 0; i < datasourcedata.length; i++) {
                    let dataitem = datasourcedata[i].start;          //this stores the starting date in a temporary letiable, if you want to store description write datasourcedata[i].description
            ...
            <script type="text/javascript">
                ...
                $(function () {
                    ...
                    function scheduler_change(e) {
                        let start = e.start; //selection start date
                        let end = e.end; //selection end date
                        let events = e.events;
                        ...
                        if (events.length) {
                            ds.fetch(function () {
                                dataItem = ds.get(parseInt(events[events.length - 1].id, 10));  //links the item clicked on to letiable dataItem
                            });
                    ...
                    function fitWidget() {
                        let widget = $("#scheduler").data("kendoScheduler");
                    ...
                    let schedDateLink = $('ul.k-scheduler-navigation').find('.k-nav-current');
                    ...
                    schedDateLink.on('click', function () {
                        ...
                        setTimeout(function () {
                            let schedCalendar = $('.k-scheduler-calendar.k-widget.k-calendar').data('kendoCalendar');
                            ...
                            schedCalendar.setOptions({
                                ...
                                open: function () {
                                    let dateViewCalendar = this.dateView.calendar;
                                    if (dateViewCalendar) {
